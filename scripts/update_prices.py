...
After the main update, estimates prices for Year 9 and Breakout items
using weighted historical averages.
"""
...
# Collections that get estimated prices instead of real scrape data
ESTIMATED_COLLECTIONS = {"Year 9", "Breakout"}
...
def estimate_price_from_history(name, hist1, hist2):
        """
            Compute a weighted average of the item's historical prices.
                More recent prices are weighted more heavily using linear decay.
                    Returns None if no history exists.
                        """
    history = hist1.get(name) or hist2.get(name)
    if not history:
                return None
...
def apply_updates(catalog, today):
    ...
    for item in items:
                ...
                # --- Real price update (existing logic) ---
                if name in catalog and catalog[name]["price"] is not None:
                                rec = catalog[name]
        ...
        # --- Estimated price update for Year 9 / Breakout ---
        item_collection = item.get("collection", "")
        # collection field may be a string or list
        if isinstance(item_collection, list):
                        in_estimated = any(c in ESTIMATED_COLLECTIONS for c in item_collection)
else:
            in_estimated = item_collection in ESTIMATED_COLLECTIONS

        if not in_estimated:
                        continue

        est = estimate_price_from_history(name, hist1, hist2)
        if est is None:
                        continue

        item["price"] = est
        # Append to whichever history file already tracks this item
        if name in hist1:
                        hist1[name][today] = est
elif name in hist2:
            hist2[name][today] = est
        estimated += 1

    print(f"  updated {updated} items (real prices)")
    print(f"  estimated {estimated} items (Year 9 / Breakout)")
...
def main():
    ...
    if len(catalog) < 100:
                ...
                sys.exit(1)
            updated, estimated = apply_updates(catalog, today)
    if updated == 0:
                print("WARNING: no items matched (name mismatch?)")
                sys.exit(1)
            print(f"Done. {updated} real + {estimated} estimated items updated for {today}.")
